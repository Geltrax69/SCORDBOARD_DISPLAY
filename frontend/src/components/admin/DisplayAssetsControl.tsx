...
import { Megaphone, Image as ImageIcon, Send, Trash2, Loader, Upload, Check, Tv } from 'lucide-react'
import {
  listDisplayAssets, createDisplayAsset, deleteDisplayAsset, showDisplayAsset, uploadTeamLogo,
} from '@/services/api'
...
export function DisplayAssetsControl() {
  ...
  const onPickImage = async (file?: File) => {
    if (!file) return
    setUploading(true)
    try { setImageUrl(await uploadTeamLogo(file)) }
    finally { setUploading(false) }
  }

  const canSave = tab === 'sponsor' ? !!imageUrl && !!title.trim() : !!body.trim()
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-dark-750 bg-dark-900/60 p-4 space-y-3">
        {tab === 'sponsor' ? (
          ...
          <>
            <input value={title} onChange={(e) => setTitle(e.target.value)}
              placeholder="Headline (optional, e.g. Final starts soon)"
              className="w-full px-3.5 py-2.5 rounded-lg text-dark-100 text-sm bg-dark-925 border border-dark-700
                         focus:outline-none focus:border-brand-500 placeholder-dark-600" />
        ...
        <div className="flex items-center gap-3">
          ...
          {imageUrl ? (
            ...
            <button onClick={() => fileRef.current?.click()} disabled={uploading}
              ...
              {tab === 'sponsor' ? 'Upload sponsor image' : 'Add image (optional)'}
            </button>
          )}
          <div className="flex items-center gap-1.5 ml-auto text-xs text-dark-500">
            <span>Hold</span>
            <input type="number" min={3} max={60} value={duration}
              onChange={(e) => setDuration(Math.max(3, Math.min(60, +e.target.value || 10)))}
        ...
        {loading ? (
          ...
          <div key={a.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl border border-dark-700 bg-dark-800">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-dark-100 truncate">{a.title || a.body || 'Untitled'}</p>
              {a.type === 'announcement' && a.title && (
                <p className="text-xs text-dark-500 truncate">{a.body}</p>
              )}
              <p className="text-xs text-dark-600">{a.duration}s</p>
            </div>
